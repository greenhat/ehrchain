...
class InMemoryContractStorageSpec extends PropSpec
  ...
  property("add and retrieve one append contract") {
    ...
    forAll(ehrAppendContractUnlimitedGen) { b: AppendContract =>
      storage.add(Seq(b))
      storage.contractsForPatient[AppendContract](b.patientPK, b.providerPK) shouldEqual Seq(b)
    }
  }
  ...
  property("add and retrieve two append contracts") {
    ...
    forAll(ehrAppendContractUnlimitedGen) { b: AppendContract =>
      storage.add(Seq(b, b))
      storage.contractsForPatient[AppendContract](b.patientPK, b.providerPK) shouldEqual Seq(b, b)
    }
  }
}
